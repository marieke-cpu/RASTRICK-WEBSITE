...
(function(){
  ...
  if (dot && ring && matchMedia('(hover: hover)').matches){
    ...
    loop();
    // Hover listeners over interactive elements — deferred to idle
    idleQ(() => {
  ...
  if (clock || clock2 || clockMobile){
    ...
    tickClock();
    let clockHandle = setInterval(tickClock, 1000);
    _pauseables.push({
      pause:  () => { clearInterval(clockHandle); clockHandle = 0; },
      resume: () => { if (!clockHandle) { tickClock(); clockHandle = setInterval(tickClock, 1000); } }
    });
  }
  ...
  idleQ(() => {
    ...
    if (stage && !window.matchMedia('(min-width: 721px)').matches){
      ...
      new ResizeObserver(() => { stageRect = stage.getBoundingClientRect(); }).observe(stage);
      ...
      cards.forEach(card => {
        ...
        card.addEventListener('pointermove', e => {
          ...
          x = Math.max(-40, Math.min(stageRect.width  - card.offsetWidth  + 40, x));
          y = Math.max(-40, Math.min(stageRect.height - card.offsetHeight + 40, y));
    ...
    if (pkgSlider) {
      ...
      pkgSlider.addEventListener('mousemove', e => {
        if (!isDown) return;
        e.preventDefault();
        pkgSlider.scrollLeft = scrollLeft - (e.pageX - pkgSlider.offsetLeft - startX) * 1.2;
      });
    ...
    if (form){
      form.addEventListener('submit', () => {
        ...
        if (btn){ btn.disabled = true; btn.innerHTML = '<span>Sending…</span><span>↗</span>'; }
      });
    }
    ...
    function loadCalendly(cb) {
      if (calendlyLoaded) { cb(); return; }
      calendlyPending.push(cb);
      if (calendlyPending.length > 1) return; // already loading
      const s = document.createElement('script');
      s.src   = 'https://assets.calendly.com/assets/external/widget.js';
      s.async = true;
      s.onload = () => {
        calendlyLoaded = true;
        const queue = calendlyPending.splice(0);
        queue.forEach(fn => fn());
      };
